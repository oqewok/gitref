...
var commands = {
  ...
  11: {
    ...
    payload: {
      hash: 'ff5dac2',
      message: 'Добавлено первое задание'
    }
  },
  12: {
    animation: gitLog,
...
function showLinkToNextStep(command) {
  return new Promise(function(resolve, reject) {
    $('.instructions > .step' + command.id + ' .next-step').fadeIn(300, function() {
      $(this).removeClass('hidden')
      resolve()
    })
  })
...
function showNextStep(commandId) {
  return new Promise(function(resolve, reject) {
    $('.instructions > .step' + commandId).fadeOut(300, function() {
      $('.instructions > .step' + (commandId + 1)).fadeIn(300, function() {
        $(this).removeClass('hidden')
        resolve()
      })
    })
  ...
  return new Promise(function(resolve, reject) {
    $commit.find('.commit-node').animate(
      ...
      function() {
        ...
        $spacer.delay(300).prependTo($commitArea).slideDown(700)

        $commit.delay(300).animate({ left: width + 'px' }, 700, function() {
          $spacer.remove()
          $(this).css('left', 0).prependTo($commitArea)
          $(this).find('.commit-message').delay(300).fadeIn(300, function() {
            resolve()
          })
        })
      }
...
function gitInit() {
  return new Promise(function(resolve, reject) {
    $('.area').removeClass('hidden').animate({ height: '100%' }, 700, function() {
      $('.area-title').fadeIn(300, function() {
        $(this).removeClass('hidden')
        resolve()
      })
    })
